...
abstract type AbstractTestStrategy <: CTModels.Strategies.AbstractStrategy end
...
CTModels.Strategies.id(::Type{TestStrategyA}) = :test_a
CTModels.Strategies.id(::Type{TestStrategyB}) = :test_b

CTModels.Strategies.metadata(::Type{TestStrategyA}) = CTModels.Strategies.StrategyMetadata(
    OptionDefinition(
        name = :max_iter,
        type = Int,
        default = 100,
...
CTModels.Strategies.metadata(::Type{TestStrategyB}) = CTModels.Strategies.StrategyMetadata(
    OptionDefinition(
        name = :backend,
        type = Symbol,
        default = :default,
...
CTModels.Strategies.options(s::Union{TestStrategyA, TestStrategyB}) = s.options
...
function test_configuration()
    Test.@testset "Strategy Configuration" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "build_strategy_options" begin
            ...
            opts = CTModels.Strategies.build_strategy_options(TestStrategyA)
            ...
            opts2 = CTModels.Strategies.build_strategy_options(TestStrategyA; max_iter=200)
            Test.@test opts2[:max_iter] == 200
            Test.@test opts2[:tolerance] == 1e-6
            
            # Multiple user values
            opts3 = CTModels.Strategies.build_strategy_options(
            ...
            # Alias resolution
            opts4 = CTModels.Strategies.build_strategy_options(TestStrategyA; max=150)
            Test.@test opts4[:max_iter] == 150
            
            opts5 = CTModels.Strategies.build_strategy_options(TestStrategyA; tol=1e-10)
            Test.@test opts5[:tolerance] == 1e-10
            
            # Different strategy
            opts6 = CTModels.Strategies.build_strategy_options(TestStrategyB; backend=:sparse)
        ...
        Test.@testset "resolve_alias" begin
            meta = CTModels.Strategies.metadata(TestStrategyA)
            
            # Primary name returns itself
            Test.@test CTModels.Strategies.resolve_alias(meta, :max_iter) == :max_iter
            Test.@test CTModels.Strategies.resolve_alias(meta, :tolerance) == :tolerance
            Test.@test CTModels.Strategies.resolve_alias(meta, :verbose) == :verbose
            
            # Aliases resolve to primary name
            Test.@test CTModels.Strategies.resolve_alias(meta, :max) == :max_iter
            Test.@test CTModels.Strategies.resolve_alias(meta, :maxiter) == :max_iter
            Test.@test CTModels.Strategies.resolve_alias(meta, :tol) == :tolerance
            
            # Unknown key returns nothing
            Test.@test CTModels.Strategies.resolve_alias(meta, :unknown) === nothing
            Test.@test CTModels.Strategies.resolve_alias(meta, :invalid) === nothing
        end
        ...
        Test.@testset "filter_options" begin
            ...
            # Filter single key
            filtered1 = CTModels.Strategies.filter_options(opts, :debug)
            Test.@test filtered1 == (max_iter=100, tolerance=1e-6, verbose=true)
            Test.@test !haskey(filtered1, :debug)
            
            # Filter multiple keys
            filtered2 = CTModels.Strategies.filter_options(opts, (:debug, :verbose))
            ...
            filtered4 = CTModels.Strategies.filter_options(opts, :nonexistent)
        ...
        Test.@testset "suggest_options" begin
            ...
            suggestions1 = CTModels.Strategies.suggest_options(:max_it, TestStrategyA)
            Test.@test :max_iter in suggestions1 || :max in suggestions1
            
            # Similar to alias
            suggestions2 = CTModels.Strategies.suggest_options(:tolrance, TestStrategyA)
            Test.@test :tolerance in suggestions2 || :tol in suggestions2
            
            # Limit suggestions
            suggestions3 = CTModels.Strategies.suggest_options(:x, TestStrategyA; max_suggestions=2)
            Test.@test length(suggestions3) <= 2
            
            # Returns vector of symbols
            suggestions4 = CTModels.Strategies.suggest_options(:unknown, TestStrategyA)
        ...
        Test.@testset "Integration: Configuration pipeline" begin
            ...
            opts = CTModels.Strategies.build_strategy_options(
                TestStrategyA;
            ...
            filtered = CTModels.Strategies.filter_options(raw_opts, :verbose)
