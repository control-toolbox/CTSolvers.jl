function (discretizer::CollocationMethod{T})(
    ocp::CTModels.Model,
) where {T<:AbstractCTScheme}

    scheme_ctdirect = SchemeSymbol[T]

    function build_adnlp_model(
        initial_guess::InitialGuess; kwargs...
    ...
    function adnlp_solution_helper(
        nlp_solution::SolverCore.AbstractExecutionStats, val::Symbol,
    )
        return nothing
    end

    function build_exa_model(
        ::Type{BaseType}, initial_guess::InitialGuess; kwargs...
    ...
    return DiscretizedOptimalControlProblem(
        ...
        CTSolvers.ADNLPModelOCPSolutionHelper(adnlp_solution_helper),
        CTSolvers.ExaModelOCPSolutionHelper(exa_solution_helper),
    )
end
