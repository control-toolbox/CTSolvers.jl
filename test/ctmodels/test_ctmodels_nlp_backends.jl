...
function test_ctmodels_nlp_backends()
    ...
    # ------------------------------------------------------------------
    # ADNLPModels backends (direct calls to ADNLPModelBackend)
    # ------------------------------------------------------------------
    # These tests exercise the call
    #   (modeler::ADNLPModelBackend)(prob, initial_guess)
    ...
    # For a problem without specialized get_* methods, ADNLPModelBackend
    ...
    # ------------------------------------------------------------------
    # ExaModels backends (direct calls to ExaModelBackend, CPU)
    # ------------------------------------------------------------------
    # These tests exercise the call
    #   (modeler::ExaModelBackend)(prob, initial_guess)
    ...
    Test.@testset "ctmodels/nlp_backends: ExaModels (CPU) – Rosenbrock (BaseType=Float32, direct call)" verbose=VERBOSE showtiming=SHOWTIMING begin
        BaseType = Float32
        modeler = CTSolvers.ExaModelBackend(; base_type=BaseType)
    ...
    # For a problem without specialized get_* methods, ExaModelBackend
    ...
    # ------------------------------------------------------------------
    # Constructor-level tests for ADNLPModelBackend and ExaModelBackend
    # ------------------------------------------------------------------
    # These tests focus on the fields set by the constructors, ensuring
    # defaults and keyword arguments are wired correctly.

    Test.@testset "ctmodels/nlp_backends: ADNLPModelBackend constructor" verbose=VERBOSE showtiming=SHOWTIMING begin
        # Default constructor should use the values from ctmodels/default.jl
        backend_default = CTSolvers.ADNLPModelBackend()
        ...
        backend_manual = CTSolvers.ADNLPModelBackend(; backend=:manual, foo=1)
        Test.@test backend_manual.backend == :manual
        Test.@test backend_manual.kwargs[:foo] == 1
    end

    Test.@testset "ctmodels/nlp_backends: ExaModelBackend constructor" verbose=VERBOSE showtiming=SHOWTIMING begin
        # Default constructor should use base_type and backend from ctmodels/default.jl
        exa_default = CTSolvers.ExaModelBackend()
        ...
        exa_custom = CTSolvers.ExaModelBackend(; base_type=Float32, foo=2)
        Test.@test exa_custom.backend === CTSolvers.__exa_model_backend()
        Test.@test exa_custom.kwargs[:foo] == 2
    end

    # ------------------------------------------------------------------
    # Solution-building via ADNLPModelBackend/ExaModelBackend(prob, nlp_solution)
    # ------------------------------------------------------------------
    ...
    Test.@testset "ctmodels/nlp_backends: ADNLPModelBackend solution building" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        modeler = CTSolvers.ADNLPModelBackend()
    ...
    Test.@testset "ctmodels/nlp_backends: ExaModelBackend solution building" verbose=VERBOSE showtiming=SHOWTIMING begin
        dummy_ad_builder = CTSolvers.ADNLPModelBuilder(x -> error("unused"))
