...
println("\n📋 Test 1: route_to() Syntaxe de Base")
println("-" ^ 40)
...
println("\n📋 Test 2: Routage Simple (Sans Conflit)")
println("-" ^ 40)
...
# ============================================================================
# Tests de Conflit de Noms
# ============================================================================

println("\n📋 Test 3: Conflit de Noms (:backend)")
println("-" ^ 40)
...
try
    ...
    if modeler_backend == :sparse && solver_backend == :cpu
        println("✅ Conflit résolu correctement")
    else
        println("❌ Conflit mal résolu")
    end
...
try
    ...
    if CTSolvers.Strategies.has_option(solver, :fake_option)
        ...
        println("✅ Mode permissif fonctionne correctement")
    else
...
try
    routed = Orchestration.route_all_options(
        TEST_METHOD, TEST_FAMILIES, TEST_ACTION_DEFS, kwargs_complete, TEST_REGISTRY; mode=:strict
    )
