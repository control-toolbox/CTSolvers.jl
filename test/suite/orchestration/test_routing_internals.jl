...
Strategies.metadata(::Type{InternalCollocation}) = Strategies.StrategyMetadata(
...
Strategies.metadata(::Type{InternalADNLP}) = Strategies.StrategyMetadata(
    Options.OptionDefinition(
        name = :backend,
        type = Symbol,
        default = :dense,
...
Strategies.metadata(::Type{InternalIpopt}) = Strategies.StrategyMetadata(
    Options.OptionDefinition(
        name = :max_iter,
        type = Int,
        default = 100,
    ...
    Test.@testset "Routing Internals" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "_route_with_disambiguation!" begin
            ...
            Orchestration._route_with_disambiguation!(
                routed, :backend, disambiguations, context,
                resolved, INTERNAL_FAMILIES, INTERNAL_REGISTRY
            )
            ...
            Orchestration._route_with_disambiguation!(
                routed, :unknown_opt, disambiguations, context,
                resolved, INTERNAL_FAMILIES, INTERNAL_REGISTRY
            )
            Test.@test length(routed[:solver]) == 1
            ...
            Test.@test_throws Exceptions.IncorrectArgument Orchestration._route_with_disambiguation!(
                routed, :totally_unknown, disambiguations, context,
                resolved, INTERNAL_FAMILIES, INTERNAL_REGISTRY
            )
        ...
        Test.@testset "_route_auto!" begin
            ...
            Orchestration._route_auto!(
                routed, :grid_size, 200, context,
                resolved, INTERNAL_FAMILIES, INTERNAL_REGISTRY, :description
            )
            ...
            Test.@test_throws Exceptions.IncorrectArgument Orchestration._route_auto!(
                routed, :totally_unknown, 42, context,
                resolved, INTERNAL_FAMILIES, INTERNAL_REGISTRY, :description
            )
        ...
        Test.@testset "_route_single_option!" begin
            ...
            Orchestration._route_single_option!(
                routed, :backend, routed_opt, context,
                resolved, INTERNAL_FAMILIES, INTERNAL_REGISTRY, :description
            )
            Test.@test length(routed[:modeler]) == 1
            Test.@test routed[:modeler][1] == (:backend => :sparse)
        ...
        Test.@testset "_build_routed_result" begin
            ...
            action_opts = Dict(
                :display => Options.OptionValue(false, :user)
            )
            routed = Dict(
            ...
            routed = Dict(
                :discretizer => Pair{Symbol, Any}[(:grid_size => 200), (:method => :trapezoid)],
                :modeler => Pair{Symbol, Any}[(:backend => :sparse)],
        ...
        Test.@testset "Complete internal workflow" begin
            ...
            kwargs = (display=false, grid_size=200, backend=Strategies.route_to(adnlp=:sparse))
