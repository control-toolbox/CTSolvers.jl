...
function Strategies.metadata(::Type{<:FakeParamStrategy{P}}) where {P<:Strategies.AbstractStrategyParameter}
    return Strategies.StrategyMetadata(
        Options.OptionDefinition(name=:test_opt, type=Int, default=42, description="Test option")
    )
end

Strategies.options(s::FakeParamStrategy) = s.options

function FakeParamStrategy{P}(; mode::Symbol=:strict, kwargs...) where {P<:Strategies.AbstractStrategyParameter}
...
test_coverage_missing_parameter() = TestCoverageMissingParameter.test_coverage_missing_parameter()
