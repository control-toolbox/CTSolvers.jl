...
function test_performance_validation()
    @testset "Performance Validation" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        known_options = (max_iter=1000, tol=1e-6, print_level=0)

        unknown_options = (
            custom_option1="test1", custom_option2="test2", custom_option3="test3"
        )
        ...
        @testset "Strategy Construction Performance" begin
            ...
            println(
                "   Strict mode median: $(BenchmarkTools.prettytime(median(strict_time.times)))",
            )
            ...
            println(
                "   Permissive mode median: $(BenchmarkTools.prettytime(median(permissive_time.times)))",
            )

            # Test permissive mode with unknown options
            println("   Testing permissive mode with unknown options...")
            permissive_unknown_time = @benchmark Solvers.Ipopt(;
                $mixed_opts..., mode=:permissive
            ) samples=1000 evals=1
            println(
                "   Permissive mode + unknown median: $(BenchmarkTools.prettytime(median(permissive_unknown_time.times)))",
            )
            ...
            overhead_unknown =
                (permissive_unknown_median - strict_median) / strict_median * 100

            println("\n📈 Overhead Analysis:")
            println("   Permissive mode overhead: $(round(overhead_permissive, digits=3))%")
            println(
                "   Permissive + unknown overhead: $(round(overhead_unknown, digits=3))%"
            )
            ...
            permissive_alloc = @allocated Solvers.Ipopt(;
                known_options..., mode=:permissive
            )
        ...
        @testset "route_to() Performance" begin
            ...
            println(
                "   Single strategy median: $(BenchmarkTools.prettytime(median(single_time.times)))",
            )
            ...
            println(
                "   Multiple strategy median: $(BenchmarkTools.prettytime(median(multi_time.times)))",
            )

            # Test complex value routing
            println("   Testing complex value routing...")
            complex_time = @benchmark route_to(
                solver=[1, 2, 3], modeler=(a=1, b=2), discretizer="test"
            ) samples=10000 evals=1
            println(
                "   Complex values median: $(BenchmarkTools.prettytime(median(complex_time.times)))",
            )
        ...
        @testset "RoutedOption Performance" begin
            ...
            println(
                "   RoutedOption median: $(BenchmarkTools.prettytime(median(routed_time.times)))",
            )
            ...
            println(
                "   route_to() wrapper median: $(BenchmarkTools.prettytime(median(wrapper_time.times)))",
            )

            # Calculate wrapper overhead
            wrapper_overhead =
                (median(wrapper_time.times) - median(routed_time.times)) /
                median(routed_time.times) * 100
        ...
        @testset "Baseline Comparison" begin
            ...
            permissive_overhead =
                (permissive_median - baseline_median) / baseline_median * 100
