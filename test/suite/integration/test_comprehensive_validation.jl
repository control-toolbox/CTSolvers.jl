    ...
    Test.@testset "Strategy Construction - $(strategy_type)" begin
        ...
        Test.@testset "Direct Constructor" begin
            Test.@testset "Strict Mode" begin
                ...
                Test.@test_throws Exceptions.IncorrectArgument strategy_type(;
                ...
                try
                    ...
                    Test.@test occursin("unknown", string(e)) ||
                        occursin("unrecognized", string(e)) ||
                        occursin("Invalid", string(e)) ||
                        occursin("not defined", string(e))
                end
            end

            Test.@testset "Permissive Mode" begin
                # Known + unknown options - should work with warning
                Test.@test_warn "Unrecognized options" strategy_type(;
        ...
        Test.@testset "build_strategy()" begin
            Test.@testset "Strict Mode" begin
                ...
                Test.@test_nowarn Strategies.build_strategy(
                    strategy_id, family, registry; known_options...
                )
                ...
                Test.@test_throws Exceptions.IncorrectArgument Strategies.build_strategy(
                    strategy_id, family, registry; known_options..., unknown_options...
                )
            end

            Test.@testset "Permissive Mode" begin
                # Known + unknown options - should work
                Test.@test_warn "Unrecognized options" Strategies.build_strategy(
                    strategy_id,
                    family,
                    registry;
                    known_options...,
        ...
        Test.@testset "build_strategy_from_resolved()" begin
            ...
            Test.@testset "Strict Mode" begin
                Test.@test_nowarn Orchestration.build_strategy_from_resolved(
                    resolved, :strategy, families, registry; known_options...
                )
...
function test_invalid_mode(strategy_type::Type)
    Test.@testset "Invalid Mode Tests - $(strategy_type)" begin
        ...
        Test.@test_throws Exceptions.IncorrectArgument Strategies.build_strategy(
            :test, Strategies.AbstractStrategy, Strategies.create_registry(); mode=:invalid
        )
    end
end
    ...
    Test.@testset "Comprehensive Strict/Permissive Validation" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "Modelers" begin
            ...
            Test.@testset "Modelers.ADNLP" begin
                ...
                Test.@testset "Option Recovery" begin
                    ...
                    redirect_stderr(devnull) do
                        strategy_build = Strategies.build_strategy(
                            :adnlp,
                            Modelers.AbstractNLPModeler,
                            modeler_registry;
                            known_options...,
        ...
        Test.@testset "Integration Tests" begin
            Test.@testset "Mode Propagation" begin
                ...
                modeler2 = Strategies.build_strategy(
                    :adnlp,
                    Modelers.AbstractNLPModeler,
                    registry;
                    backend=:default,
            ...
            Test.@testset "Error Quality" begin
                ...
                try
                    ...
                    Test.@test occursin("strict", string(e)) ||
                        occursin("permissive", string(e))
                end
            end
            ...
            Test.@testset "Option Consistency" begin
                ...
                redirect_stderr(devnull) do
                    ...
                    for strategy in strategies
                        ...
                        Test.@test Strategies.option_value(strategy, :test_consistency) ==
                            42
                        Test.@test Strategies.option_source(strategy, :backend) == :user
                        Test.@test Strategies.option_source(strategy, :show_time) == :user
                        Test.@test Strategies.option_source(strategy, :test_consistency) ==
                            :user
                        # Verify mode is NOT stored in options (correct behavior)
                        Test.@test_throws Exception strategy.options.mode
                    end
