...
"""
function test_option_routing(
    strategy, option_name::Symbol, expected_value, expected_source::Symbol=:user
)
    ...
    Test.@testset "Route To Validation - Mode: $mode" begin
        if expected_success
            ...
            for (family_name, family_type) in pairs(families)
                if haskey(routed.strategies, family_name) &&
                    !isempty(routed.strategies[family_name])
                    ...
                    strategy = create_mock_strategy(
                        strategy_type; mode=mode, routed.strategies[family_name]...
                    )
    ...
    Test.@testset "Route To Comprehensive Tests" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "Mock Strategies - Option Conflicts" begin
            Test.@testset "Single Strategy Routing" begin
                ...
                discretizer = create_mock_strategy(
                    RouteCollocation; mode=:strict, routed.strategies.discretizer...
                )
                modeler = create_mock_strategy(
                    RouteADNLP; mode=:strict, routed.strategies.modeler...
                )
                solver = create_mock_strategy(
                    RouteIpopt; mode=:strict, routed.strategies.solver...
                )
                ...
                Test.@testset "Option Distribution" begin
                    ...
                    if haskey(routed.strategies, :solver) &&
                        haskey(routed.strategies.solver, :max_iter)
        ...
        Test.@testset "Validation Mode Tests" begin
            Test.@testset "Unknown Options (Default Behavior)" begin
                ...
                test_route_to_with_validation(
                    MOCK_METHOD, MOCK_FAMILIES, kwargs, expected_success=false
                )
            end
            ...
            Test.@testset "Unknown Options with Bypass" begin
                ...
                redirect_stderr(devnull) do
                    ...
                    return test_option_routing(solver, :fake_option, 123)
                end
            end
        end
        ...
        Test.@testset "Multi-Solver Tests" begin
            Test.@testset "Multiple Solvers with Conflicts" begin
                ...
                # Build strategies
                discretizer = create_mock_strategy(
                    RouteCollocation; routed.strategies.discretizer...
                )
                ...
                Test.@testset "Solver Options" begin
                    ...
                    Test.@test Strategies.has_option(ipopt, :backend) ||
                        Strategies.has_option(madnlp, :backend)
                    Test.@test Strategies.has_option(ipopt, :max_iter) ||
                        Strategies.has_option(madnlp, :max_iter)
                end
            end
        end
        ...
        Test.@testset "Edge Cases" begin
            ...
            Test.@testset "Empty RoutedOption" begin
                Test.@test_throws Exceptions.PreconditionError Strategies.RoutedOption(
                    NamedTuple()
                )
            end
        end
