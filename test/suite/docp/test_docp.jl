...
function (modeler::FakeModelerDOCP)(
    prob::DOCP.DiscretizedModel, nlp_solution::SolverCore.AbstractExecutionStats
)
    if modeler.backend == :adnlp
    ...
    Test.@testset "DOCP Module" verbose = VERBOSE showtiming = SHOWTIMING begin
        ...
        Test.@testset "DOCP.DiscretizedModel Type" begin
            Test.@testset "Construction" begin
                ...
                adnlp_sol_builder = Optimization.ADNLPSolutionBuilder(
                    s -> (objective=s.objective,)
                )
                exa_sol_builder = Optimization.ExaSolutionBuilder(
                    s -> (objective=s.objective,)
                )
            ...
            Test.@testset "Type parameters" begin
                ...
                Test.@test typeof(docp.adnlp_model_builder) <:
                    Optimization.ADNLPModelBuilder
                Test.@test typeof(docp.exa_model_builder) <: Optimization.ExaModelBuilder
                Test.@test typeof(docp.adnlp_solution_builder) <:
                    Optimization.ADNLPSolutionBuilder
                Test.@test typeof(docp.exa_solution_builder) <:
                    Optimization.ExaSolutionBuilder
            end
        end
        ...
        Test.@testset "Contract Implementation" begin
            ...
            Test.@testset "Type stability" begin
                ...
                Test.@test_nowarn Test.@inferred Optimization.get_adnlp_solution_builder(
                    docp
                )
        ...
        Test.@testset "Building Functions" begin
            ...
            adnlp_sol_builder = Optimization.ADNLPSolutionBuilder(
                s -> (objective=s.objective, status=s.status)
            )
            exa_sol_builder = Optimization.ExaSolutionBuilder(
                s -> (objective=s.objective, iter=s.iter)
            )
        ...
        Test.@testset "Integration Tests" begin
            ...
            Test.@testset "Complete DOCP workflow - Exa" begin
                ...
                exa_sol_builder = Optimization.ExaSolutionBuilder(
                    s -> (objective=s.objective, iterations=s.iter, status=s.status)
                )
