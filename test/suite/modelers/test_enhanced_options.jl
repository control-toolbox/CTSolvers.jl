    ...
    Test.@testset "Enhanced Modelers Options" verbose = VERBOSE showtiming = SHOWTIMING begin
        ...
        Test.@testset "Advanced Backend Overrides" begin
            ...
            Test.@testset "Backend Override with Type{<:ADBackend}" begin
                ...
                Test.@test Strategies.options(modeler)[:gradient_backend] ===
                    FakeTestBackend
            end
            ...
            Test.@testset "Backend Override with ADBackend instance" begin
                ...
                Test.@test Strategies.options(modeler)[:gradient_backend] isa
            ...
            Test.@testset "Backend Override Type Validation" begin
                ...
                redirect_stderr(devnull) do
                    Test.@test_throws Exceptions.IncorrectArgument Modelers.ADNLP(
                        gradient_backend="invalid"
                    )
                    Test.@test_throws Exceptions.IncorrectArgument Modelers.ADNLP(
                        hprod_backend=123
                    )
                    Test.@test_throws Exceptions.IncorrectArgument Modelers.ADNLP(
                        jprod_backend=:invalid
                    )
                    Test.@test_throws Exceptions.IncorrectArgument Modelers.ADNLP(
                        ghjvprod_backend="invalid"
                    )
                end
            end
        ...
        Test.@testset "Backend Aliases with Deprecation Warnings" begin
            ...
            Test.@testset "Depreciation warnings" begin
                redirect_stderr(devnull) do
                    Test.@test_logs (
                        :warn, "adnlp_backend is deprecated, use backend instead"
                    ) Modelers.ADNLP(adnlp_backend=:default)
                    Test.@test_logs (
                        :warn, "exa_backend is deprecated, use backend instead"
                    ) Modelers.Exa(exa_backend=nothing)
                end
            end
