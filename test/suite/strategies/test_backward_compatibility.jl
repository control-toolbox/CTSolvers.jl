...
function test_backward_compatibility()
    Test.@testset "Backward Compatibility" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "Non-parameterized strategies work" begin
            ...
            r = Strategies.create_registry(Modelers.AbstractNLPModeler => (Modelers.Exa,))
        ...
        Test.@testset "ID uniqueness maintained" begin
            ...
            Test.@test Strategies.id(Modelers.Exa{Strategies.CPU}) ==
                Strategies.id(Modelers.Exa{Strategies.GPU}) ==
                :exa
            Test.@test Strategies.id(Solvers.MadNLP{Strategies.CPU}) ==
                Strategies.id(Solvers.MadNLP{Strategies.GPU}) ==
                :madnlp
            Test.@test Strategies.id(Solvers.MadNCL{Strategies.CPU}) ==
                Strategies.id(Solvers.MadNCL{Strategies.GPU}) ==
                :madncl
        end
