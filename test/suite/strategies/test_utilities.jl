...
function test_utilities()
    Test.@testset "Strategy Utilities" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "filter_options - multiple keys" begin
            ...
            # Filter all keys
            filtered3 = Strategies.filter_options(
                opts, (:max_iter, :tolerance, :verbose, :debug)
            )
        ...
        Test.@testset "suggest_options - structured results" begin
            ...
            # Limit suggestions
            suggestions4 = Strategies.suggest_options(
                :x, TestUtilStrategy; max_suggestions=2
            )
            Test.@test length(suggestions4) <= 2

            # Single suggestion
            suggestions5 = Strategies.suggest_options(
                :unknown, TestUtilStrategy; max_suggestions=1
            )
        ...
        Test.@testset "options_dict" begin
            ...
            strategy = TestUtilStrategy(
                Strategies.build_strategy_options(
                    TestUtilStrategy; max_iter=500, tolerance=1e-8, verbose=true
                ),
            )
        ...
        Test.@testset "Integration: options_dict workflow" begin
            ...
            strategy = TestUtilStrategy(
                Strategies.build_strategy_options(
                    TestUtilStrategy; max_iter=100, tolerance=1e-6
                ),
            )
