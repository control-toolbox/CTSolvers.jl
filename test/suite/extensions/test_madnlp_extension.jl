...
function test_madnlp_extension()
    Test.@testset "MadNLP Extension" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "Metadata" begin
            ...
            Test.@test Options.type(meta[:linear_solver]) ==
            ...
            Test.@test Options.type(meta[:fixed_variable_treatment]) ==
                Type{<:MadNLP.AbstractFixedVariableTreatment}
            Test.@test Options.type(meta[:equality_treatment]) ==
                Type{<:MadNLP.AbstractEqualityTreatment}
            Test.@test Options.type(meta[:kkt_system]) ==
                Union{Type{<:MadNLP.AbstractKKTSystem},UnionAll}
            Test.@test Options.type(meta[:hessian_approximation]) ==
                Union{Type{<:MadNLP.AbstractHessian},UnionAll}
            Test.@test Options.type(meta[:inertia_correction_method]) ==
                Type{<:MadNLP.AbstractInertiaCorrector}
        ...
        Test.@testset "GPU Tests" begin
            if is_cuda_on()
                ...
                Test.@testset "Elec - GPU" begin
                    elec = TestProblems.Elec()
                    sol = CommonSolve.solve(
                        elec.prob, elec.init, gpu_modeler, gpu_solver; display=false
                    )
                    Test.@test sol.status == MadNLP.SOLVE_SUCCEEDED
                ...
                Test.@testset "Max1MinusX2 - GPU" begin
                    max_prob = TestProblems.Max1MinusX2()
                    sol = CommonSolve.solve(
                        max_prob.prob, max_prob.init, gpu_modeler, gpu_solver; display=false
                    )
                    Test.@test sol.status == MadNLP.SOLVE_SUCCEEDED
        ...
        Test.@testset "Termination Options Validation" begin
            ...
            redirect_stderr(devnull) do
                Test.@test_throws Exceptions.IncorrectArgument Solvers.MadNLP(
                    acceptable_tol=-1.0
                )
                Test.@test_throws Exceptions.IncorrectArgument Solvers.MadNLP(
                    acceptable_tol=0.0
                ...
                Test.@test_throws Exceptions.IncorrectArgument Solvers.MadNLP(
                    diverging_iterates_tol=-1.0
                )
                Test.@test_throws Exceptions.IncorrectArgument Solvers.MadNLP(
                    diverging_iterates_tol=0.0
                )
            end

            # Test valid values work (suppress solver output)
            Test.@test_nowarn Solvers.MadNLP(
                acceptable_tol=1e-5, acceptable_iter=10, print_level=MadNLP.ERROR
            )
            Test.@test_nowarn Solvers.MadNLP(max_wall_time=60.0, print_level=MadNLP.ERROR)
            Test.@test_nowarn Solvers.MadNLP(
                diverging_iterates_tol=1e10, print_level=MadNLP.ERROR
            )
        end
        ...
        Test.@testset "NLP Scaling Options Validation" begin
            ...
            Test.@test_nowarn Solvers.MadNLP(
                nlp_scaling_max_gradient=100.0, print_level=MadNLP.ERROR
            )
            Test.@test_nowarn Solvers.MadNLP(
                jacobian_constant=true, print_level=MadNLP.ERROR
            )
            Test.@test_nowarn Solvers.MadNLP(
                hessian_constant=true, print_level=MadNLP.ERROR
            )
        ...
        Test.@testset "Initialization Options Validation" begin
            ...
            Test.@test_nowarn Solvers.MadNLP(
                constr_mult_init_max=1000.0, print_level=MadNLP.ERROR
            )

            # Test Type values
            Test.@test_nowarn Solvers.MadNLP(
                fixed_variable_treatment=MadNLP.MakeParameter, print_level=MadNLP.ERROR
            )
            Test.@test_nowarn Solvers.MadNLP(
                equality_treatment=MadNLP.RelaxEquality, print_level=MadNLP.ERROR
            )
        ...
        Test.@testset "Advanced Options Validation" begin
            ...
            Test.@test_nowarn Solvers.MadNLP(
                kkt_system=MadNLP.SparseKKTSystem, print_level=MadNLP.ERROR
            )
            Test.@test_nowarn Solvers.MadNLP(
                hessian_approximation=MadNLP.BFGS, print_level=MadNLP.ERROR
            )
            Test.@test_nowarn Solvers.MadNLP(
                inertia_correction_method=MadNLP.InertiaAuto, print_level=MadNLP.ERROR
            )
            ...
            redirect_stderr(devnull) do
                ...
                Test.@test_throws Exceptions.IncorrectArgument Solvers.MadNLP(
                    hessian_approximation=1.0
                )
                Test.@test_throws Exceptions.IncorrectArgument Solvers.MadNLP(
                    inertia_correction_method="invalid"
                )
        ...
        Test.@testset "Multiple Solves" begin
            ...
            Test.@test Symbol(stats1.status) in
                (:SOLVE_SUCCEEDED, :SOLVED_TO_ACCEPTABLE_LEVEL)
            Test.@test Symbol(stats2.status) in
                (:SOLVE_SUCCEEDED, :SOLVED_TO_ACCEPTABLE_LEVEL)
        end
        ...
        Test.@testset "solve_with_madnlp Function" begin
            ...
            modelers_names = ["Modelers.ADNLP", "Modelers.Exa (CPU)"]
            madnlp_options = Dict(
                :max_iter => 1000, :tol => 1e-6, :print_level => MadNLP.ERROR
            )
            linear_solvers = [MadNLP.UmfpackSolver, MadNLP.MumpsSolver]
            linear_solver_names = ["Umfpack", "Mumps"]
        ...
        Test.@testset "GPU - Initial Guess (max_iter=0)" begin
            if is_cuda_on()
                ...
                Test.@testset "Rosenbrock - GPU" begin
                    ros = TestProblems.Rosenbrock()
                    sol = CommonSolve.solve(
                        ros.prob, ros.sol, gpu_modeler, gpu_solver_0; display=false
                    )
                    Test.@test sol.status == MadNLP.MAXIMUM_ITERATIONS_EXCEEDED
                    Test.@test Array(sol.solution) ≈ ros.sol atol=1e-6
                end
                ...
                Test.@testset "Elec - GPU" begin
                    elec = TestProblems.Elec()
                    sol = CommonSolve.solve(
                        elec.prob, elec.init, gpu_modeler, gpu_solver_0; display=false
                    )
                    Test.@test sol.status == MadNLP.MAXIMUM_ITERATIONS_EXCEEDED
